...
def report(args, logger):
    ...
    if gitstore.check_if_git_dir_exist():
        logger.debug('Found destination git directory: %s' % args.git_dir)
...
def register_commands(subparsers):
    ...
    parser_build.add_argument('-d', '--git_dir', default='',
                              help='(Optional) Source directory to be used as git repository '
                                   'to compute test report. Default location for source directory '
                                   'will be <top_dir>/testresults.git.')
